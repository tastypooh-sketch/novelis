            ...
            <div className="relative z-10 w-full max-w-md p-8 rounded-xl shadow-2xl border" style={{ backgroundColor: settings.toolbarBg, borderColor: settings.toolbarInputBorderColor }}>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        ...
                        {error && <p className="text-app-danger text-xs mt-2 text-center">{error}</p>}
                    </div>

                    <button 
                        type="submit" 
                        disabled={!licenseKey.trim()}
                        className="w-full py-3 rounded-lg font-bold text-app-text shadow-lg transition-transform active:scale-95 disabled:opacity-50 disabled:scale-100"
