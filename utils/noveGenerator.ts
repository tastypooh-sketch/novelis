...
export const generateNoveHTML = (state: INovelState, settings: EditorSettings, writingGoals: WritingGoals, faviconBase64: string = ""): string => {
...
<head>
    ...
    <style>
        ...
        @keyframes flash-accent-glow {
            0% { filter: drop-shadow(0 0 10px ${settings.accentColor || '#4A90E2'}) brightness(1.2); }
            100% { filter: none; }
        }
...
<body class="h-screen w-screen flex flex-col transition-colors duration-300 overflow-hidden" id="body-root">
    ...
    <script type="text/babel">
        ...
        const generateSimpleRtf = (chapter) => {
            ...
            const rtfHeader = '{\\\\rtf1\\\\ansi\\\\deff0 {\\\\fonttbl {\\\\f0 Times New Roman;}} \\\\f0\\\\fs24 \\\\pard\\\\qc\\\\b\\\\fs32 ';
            const rtfBody = ' \\\\par\\\\par \\\\pard\\\\fi360 ';
            return rtfHeader + escape(chapter.title) + ' ' + chapter.chapterNumber + rtfBody + escape(text) + ' }';
        };
        ...
        const Icons = {
            ...
            History: () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h6m6A12 12 0 112.89 6.89" /></svg>,
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm" onClick={onClose}>
                    <div className="p-6 rounded-lg shadow-xl w-full max-w-lg flex flex-col" style={{ backgroundColor: settings.toolbarBg, color: settings.toolbarText }} onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex gap-2 mb-4">
                            ...
                            <button onClick={handleAdd} className="px-4 py-2 rounded font-bold" style={{ backgroundColor: settings.accentColor, color: '#FFFFFF' }}>Add</button>
                        </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-md">
                    <div className="w-full max-w-xl m-4 rounded-lg shadow-2xl border flex flex-col overflow-hidden" style={{ backgroundColor: settings.toolbarBg, color: settings.textColor, borderColor: settings.toolbarInputBorderColor }}>
                        ...
                        <footer className="p-6 border-t flex justify-end gap-4" style={{ borderColor: settings.toolbarInputBorderColor }}>
                            <button disabled={!accepted} onClick={onAccept} className="px-8 py-2 rounded font-bold shadow-lg transition-all active:scale-95 disabled:opacity-30" style={{ backgroundColor: settings.accentColor, color: '#FFFFFF' }}>Accept & Proceed</button>
                        </footer>
                    </div>
                </div>
            );
                        ...
                        <div className="p-8 overflow-y-auto flex-grow space-y-10 text-sm leading-relaxed scroll-smooth">
                            ...
                            <div className="p-6 mt-6 rounded-xl border-2 text-center space-y-3 bg-black/10" style={{ borderColor: settings.accentColor }}>
                                ...
                                <a 
                                    ...
                                    className="inline-block px-6 py-2 rounded-lg font-bold transition-transform active:scale-95"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-6">
                                    ...
                                    <div>
                                     ...
                                     <button onClick={handleSave} className="w-full px-4 py-2 rounded-md font-bold" style={{ backgroundColor: settings.successColor || '#16a34a', color: '#FFFFFF' }}>Save Goals</button>
                                </div>
                                <div>
                ...
                <div className="fixed z-50 w-80 rounded-lg shadow-2xl border flex flex-col" style={{top: position.y, left: position.x, backgroundColor: settings.toolbarBg, borderColor: settings.toolbarInputBorderColor, color: settings.textColor}}>
                    ...
                    <div className="p-3 space-y-3" style={{backgroundColor: settings.backgroundColor}}>
                        ...
                        <div className="flex gap-2 pt-2 border-t" style={{borderColor: settings.toolbarInputBorderColor}}>
                            ...
                            <button onClick={() => onReplaceAll(find, replaceText, scope)} className="flex-1 py-1.5 text-xs rounded hover:opacity-90" style={{backgroundColor: settings.accentColor, color: '#FFFFFF'}}>Replace All</button>
                        </div>
                    </div>
        ...
        const VersionHistoryModal = ({ onClose, settings, dirHandle, activeChapter, onRestore }) => {
            ...
            useEffect(() => {
                const fetchHistory = async () => {
                    if (!dirHandle) {
                        setError("Connect to a project folder to enable version history.");
                        setIsLoading(false);
                        return;
                    }
                    try {
                        const historyHandle = await dirHandle.getDirectoryHandle('Cumulative Saves', { create: false });
                        const chapterPrefix = activeChapter.title + '-' + activeChapter.chapterNumber + '_';
                        const found = [];
                        // @ts-ignore
                        for await (const entry of historyHandle.values()) {
                            if (entry.kind === 'file' && entry.name.startsWith(chapterPrefix) && entry.name.endsWith('.rtf')) {
                                ...
                                found.push({ name: entry.name, timestamp: dateObj, handle: entry });
                            }
                        }
                        found.sort((a, b) => b.timestamp - a.timestamp);
                        setVersions(found);
                    } catch (e) {
                        if (e.name === 'NotFoundError') setError("No 'Cumulative Saves' folder found. History will appear here once you save changes.");
                        else setError("Could not read history.");
                    } finally {
                        setIsLoading(false);
                    }
                };
                fetchHistory();
            }, [dirHandle, activeChapter]);
            ...
            const loadVersionContent = async (version) => {
                ...
                try {
                    ...
                    content = content.replace(/\\\\pars?/g, '\\\\n');
                    content = content.replace(/\\\\lines?/g, '\\\\n');
                    ...
                    setSelectedVersion({ ...version, content: plainText });
                } catch (e) {
            ...
            return (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm p-4">
                    <div className="w-full max-w-4xl rounded-lg shadow-2xl flex flex-col" style={{ backgroundColor: settings.toolbarBg, color: settings.textColor, maxHeight: '90vh' }}>
                        <header className="flex justify-between items-center p-4 border-b" style={{ borderColor: settings.toolbarButtonBg }}>
                            <h2 className="text-lg font-semibold">History for {activeChapter.title}</h2>
                            <button onClick={onClose}><Icons.Close /></button>
                        </header>
                        <main className="flex-grow p-6 overflow-hidden grid grid-cols-3 gap-6 min-h-[50vh]">
                            <div className="col-span-1 border-r pr-4 flex flex-col overflow-hidden" style={{borderColor: settings.toolbarButtonBg}}>
                                <h3 className="font-semibold mb-2">Versions</h3>
                                <div className="overflow-y-auto space-y-1">
                                    {isLoading && <div className="flex items-center gap-2"><Icons.Spinner /><span>Loading...</span></div>}
                                    ...
                                    {versions.map(v => (
                                        <button
                                            ...
                                            style={{backgroundColor: selectedVersion?.name === v.name ? settings.accentColor : 'transparent', color: selectedVersion?.name === v.name ? '#FFF' : settings.textColor}}
                                        >
                            ...
                            <div className="col-span-2 flex flex-col overflow-hidden">
                                <h3 className="font-semibold mb-2">Preview (Plain Text)</h3>
                                <div className="flex-grow p-4 rounded border overflow-y-auto whitespace-pre-wrap text-sm" style={{backgroundColor: settings.backgroundColor, borderColor: settings.toolbarButtonBg}}>
                                    {selectedVersion?.content ?? (selectedVersion ? "Loading..." : "Select a version to preview.")}
                                </div>
                            </div>
                        </main>
                        <footer className="p-4 border-t flex justify-end gap-3" style={{ borderColor: settings.toolbarButtonBg }}>
                            <button onClick={onClose} className="px-4 py-2 rounded border" style={{ borderColor: settings.toolbarButtonBg }}>Close</button>
                            <button onClick={handleRestore} disabled={!selectedVersion?.content} className="px-4 py-2 rounded disabled:opacity-50" style={{ backgroundColor: settings.accentColor, color: '#FFF' }}>Restore Version</button>
                        </footer>
                    </div>
                </div>
            );
        ...
        const NoveManuscript = ({ chapter, onChange, onRenameTitle, settings, isFocusMode, onPlaySound, notesOpen, onPageInfoChange, isSpellcheckEnabled, searchTarget }) => {
            ...
            useEffect(() => { 
                if (onPageInfoChange) onPageInfoChange(pageInfo); 
            ...
            useLayoutEffect(() => {
                const isChapterChange = prevChapterId.current !== chapter.id;
                    ...
                    if (editorRef.current) {
                        ...
                        if (sel) {
                            try {
                                ...
                                console.warn("Failed to set selection at start of chapter", e);
                            }
                        }
                    }
                }
                            ...
                            <div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex gap-2">
                                        <input type="text" placeholder="Image URL" value={newUrl} onChange={e => setNewUrl(e.target.value)} className="flex-grow px-2 py-1 rounded border" style={{ backgroundColor: '#FFFFFF', color: '#000000' }} />
                                        <button onClick={() => onSettingsChange({ backgroundImage: newUrl })} className="px-3 py-1 rounded" style={{backgroundColor: settings.accentColor, color: '#FFFFFF'}}>Set</button>
        ...
        const NoveApp = () => {
            ...
            const [hasAcceptedEULA, setHasAcceptedEULA] = useState(() => {
                try {
                    return localStorage.getItem('noveli_eula_accepted') === 'true';
                } catch (e) {
            ...
            useEffect(() => {
                const autoLoadOnGesture = async () => {
                    try {
                        ...
                        if (handle) {
                            ...
                            if (status === 'granted') {
                                ...
                                // Browsers require a user gesture to upgrade 'prompt' to 'granted'
                                const trigger = async () => {
                                    try {
                    ...
                    } catch (e) { console.error("Auto-load initialization failed", e); }
                };
                autoLoadOnGesture();
            }, []);
            ...
            const handleAcceptEULA = () => {
                ...
                try {
                    ...
                    console.warn("Could not save EULA acceptance to localStorage", e);
                }
            };
                    ...
                    if ('showDirectoryPicker' in window) {
                        try {
                            ...
                            if (!dirHandle || forceNewFolder) { 
                                ...
                                // Ensure we have permission after page reload
                            ...
                            // SAVE CHAPTER HISTORY (RTF)
                            try {
                                const historyDir = await dirHandle.getDirectoryHandle('Cumulative Saves', { create: true });
                                const now = new Date();
                                const timestamp = now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-' + String(now.getDate()).padStart(2, '0') + '_' + String(now.getHours()).padStart(2, '0') + '-' + String(now.getMinutes()).padStart(2, '0') + '-' + String(now.getSeconds()).padStart(2, '0');
                                const historyFilename = activeChapter.title + '-' + activeChapter.chapterNumber + '_' + timestamp + '.rtf';
                                const historyHandle = await historyDir.getFileHandle(historyFilename, { create: true });
                                const historyWritable = await historyHandle.createWritable();
                                await historyWritable.write(generateSimpleRtf(activeChapter));
                                await historyWritable.close();
                            } catch (e) { console.error("History save failure", e); }
            ...
            const isBlankProject = useMemo(() => {
                return chapters.length === 1 && 
                       (chapters[0].title === 'Chapter 1' || chapters[0].title === 'Chapter') && 
                       (chapters[0].content === '<div><br></div>' || chapters[0].content === '');
            }, [chapters]);
            ...
            return (
                <div className={'flex flex-col h-screen font-sans overflow-hidden transition-colors duration-300 bg-transparent ' + (isSaveFlaring ? 'animate-save-flare' : '')} style={{ '--flare-color': settings.accentColor }}>
                    ...
                    <div className="flex flex-grow overflow-hidden relative">
                        ...
                        {isHistoryOpen && <VersionHistoryModal onClose={() => setIsHistoryOpen(false)} settings={settings} dirHandle={portableDirHandle} activeChapter={activeChapter} onRestore={(content) => handleContentChange(content)} />}
                        {notesOpen && !isFocusMode && (
                            <div className="w-80 border-l flex flex-col shadow-xl z-20 flex-shrink-0 transition-colors backdrop-blur-md" style={{backgroundColor: settings.toolbarBg + 'E6', borderColor: settings.toolbarButtonBg}}>
                                ...
                                <div className="p-4 flex-grow overflow-y-auto space-y-4">
                                    ...
                                    <div className="mt-auto pt-4 border-t" style={{borderColor: settings.toolbarButtonBg}}><div className="p-3 rounded-lg text-center" style={{backgroundColor: settings.backgroundColor + '80'}}><p className="text-xs opacity-70 mb-2">Need AI tools?</p><button className="w-full py-1.5 rounded text-xs font-bold shadow-sm hover:opacity-90 transition-opacity cursor-not-allowed opacity-50" disabled style={{backgroundColor: settings.accentColor, color: '#FFFFFF'}}>Available in Novelis Desktop</button></div></div>
                                </div>
                            </div>
                        )}
                    </div>
                    <div className={\`flex-shrink-0 flex flex-wrap items-center justify-center sm:justify-between gap-y-2 px-4 shadow-lg z-20 transition-all duration-300 \${isFocusMode ? 'max-h-0 py-0 opacity-0 pointer-events-none border-none overflow-hidden' : 'max-h-48 py-2 opacity-100 border-t'}\`} style={{backgroundColor: settings.toolbarBg, borderColor: settings.toolbarButtonBg}}>
                        ...
                        <div className="flex items-center gap-2 flex-wrap justify-center sm:justify-end w-full sm:w-auto">
                            ...
                            <button onClick={() => setIsHistoryOpen(true)} className="p-2 rounded hover:opacity-80 transition-colors" style={isHistoryOpen ? activeBtnStyle : btnStyle} title="Chapter Version History (from Cumulative Saves folder)"><Icons.History /></button>
                            ...
                            <div className="flex items-center gap-1 bg-black/10 p-1 rounded-lg border border-white/5 ml-2">
                                ...
                                <div className="relative flex items-center">
                                    <button 
                                        ...
                                        className="flex items-center gap-2 px-3 py-1.5 rounded text-sm font-medium transition-colors hover:opacity-90 group" 
                                        style={{backgroundColor: settings.accentColor, color: '#FFFFFF'}} 
                                    ...
                                    {isSaveMenuOpen && (
                                        ...
                                        >
                                            <button 
                                                ...
                                                className="w-full text-left px-3 py-2 rounded-lg text-xs font-semibold hover:bg-white/10 flex items-center gap-2 transition-colors"
                                                style={{ color: '#FFFFFF' }}
                                            >
                                                <Icons.Save />
                    ...
                    {isFocusMode && <button onClick={() => setIsFocusMode(false)} className="absolute top-4 right-4 p-2 rounded-full bg-black/20 hover:bg-black/40 transition-colors z-50" style={{ color: '#FFFFFF' }} title="Exit Focus Mode"><Icons.Unfocus /></button>}
                </div>
            );
        };
        ...
        root.render(<NoveApp />);
        window.NoveBoot.loaded = true;
        console.log("Nové Rendered Successfully in " + (Date.now() - window.NoveBoot.start) + "ms");
        
        // Final cleanup of loading screen just in case
        setTimeout(() => {
