...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // AI Support
  callAI: (params, headers) => ipcRenderer.invoke('call-ai', params, headers),

  // Dirty State Management
  updateDirtyStatus: (isDirty) => ipcRenderer.send('update-dirty-status', isDirty),
  forceClose: () => ipcRenderer.send('force-close-app'),
  onSaveAndExit: (callback) => {
    const subscription = (event) => callback();
    ipcRenderer.on('request-save-and-exit', subscription);
    return () => ipcRenderer.removeListener('request-save-and-exit', subscription);
  },
  onRequestCloseConfirmation: (callback) => {
    const subscription = (event) => callback();
    ipcRenderer.on('request-close-confirmation', subscription);
    return () => ipcRenderer.removeListener('request-close-confirmation', subscription);
  }
});
