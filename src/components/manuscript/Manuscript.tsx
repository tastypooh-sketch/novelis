...
import type { IChapter, EditorSettings, Shortcut, WritingGoals, GalleryItem, INovelState, SearchResult, AppUpdate, INoteItem } from '../../types';
...
const createWavUrl = (pcmData: Uint8Array, sampleRate: number): string => {
    ...
    const blob = new Blob([header, pcmData as any], { type: 'audio/wav' });
...
export const Manuscript: React.FC<ManuscriptProps> = ({ 
    ...
    writingGoals, onWritingGoalsChange, directoryHandle, projectPath, onDirectoryHandleChange,
    onGenerateWhatIf,
    ...
    const { autoCorrectDictionary } = useNovelState();
    const shortcutsMap = useMemo(() => {
        const map = new Map<string, string>();
        shortcuts.forEach(s => map.set(s.key, s.value));
        Object.entries(autoCorrectDictionary || {}).forEach(([key, value]) => map.set(key, value));
        return map;
    }, [shortcuts, autoCorrectDictionary]);
    ...
    const handleManualSave = useCallback(async () => {
        if (isSaving) return;
        const success = await onSaveToFolder(false);
        if (success) {
            const now = new Date();
    ...
    const handleSendToNotes = useCallback((selectedText: string) => {
        let currentNotes: INoteItem[] = [];
        try {
            if (activeChapter.notes) {
                const parsed = JSON.parse(activeChapter.notes);
                if (Array.isArray(parsed)) {
                    currentNotes = parsed;
                } else {
                    currentNotes = [{ id: generateId(), type: 'text', content: activeChapter.notes }];
                }
            }
        } catch (e) {
            if (activeChapter.notes) {
        ...
        handleChapterDetailsChange(activeChapter.id, { notes: JSON.stringify(updatedNotes) });
        setIsNotesPanelOpen(true);
        setNotification("Selection sent to Chapter Notes");
        setTimeout(() => setNotification(null), 2000);
    }, [activeChapter.id, activeChapter.notes, generateId, handleChapterDetailsChange]);

    const handleExportNove = useCallback(async () => {
        setIsSaving(true);
        setNotification("Preparing Nové Portable...");
        try {
            await exportForNove(fullState, settings, writingGoals);
            setNotification("Nové Portable Exported");
        } catch (e) {
            console.error("Export failed:", e);
            dialog.error("An error occurred during export.", "Export Failed");
        } finally {
            setIsSaving(false);
            setTimeout(() => setNotification(null), 3000);
        }
    ...
    const handleExportStandaloneNove = useCallback(async () => {
        setIsSaving(true);
        setNotification("Generating Standalone Nové...");
        try {
            await exportStandaloneNove(fullState, settings, writingGoals);
            setNotification("Standalone Nové Exported");
        } catch (e) {
            console.error("Export failed:", e);
            dialog.error("An error occurred during standalone export.", "Export Failed");
        } finally {
            setIsSaving(false);
            setTimeout(() => setNotification(null), 3000);
        }
    ...
    const handleExportBlankNove = useCallback(async () => {
        ...
        try {
            ...
            console.error("Export failed:", e);
            dialog.error("An error occurred during blank export.", "Export Failed");
        } finally {
            setIsSaving(false);
            setTimeout(() => setNotification(null), 3000);
        }
    ...
    const handleContextMenu = useCallback((e: React.MouseEvent) => {
        ...
        setContextMenu({ 
            ...
            y: e.clientY, 
            actions: [
                { label: '✨ Rewrite Paragraphs', onSelect: () => onRewriteParagraphs(text, context) },
                { label: '📝 Send Selection to Notes', onSelect: () => handleSendToNotes(text) },
                { label: '🔮 What If? (Brainstorm Alternatives)', onSelect: () => onGenerateWhatIf(text, context) }
            ] 
        });
    }, [onGenerateWhatIf, onRewriteParagraphs, handleSendToNotes]);
    ...
    useEffect(() => {
        // Trigger layout recalculation after transitions finish
        const timer = setTimeout(calculateLayout, 350);
        return () => clearTimeout(timer);
    }, [isFocusMode, isVisible, calculateLayout]);
        ...
        if (sel && sel.rangeCount > 0) {
            ...
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                if (nativeEvent?.inputType === 'insertText') {
                    if (insertedChar === '"' || insertedChar === "'") {
                    ...
                    } else if (insertedChar === ' ' || insertedChar === null) {
                        ...
                        if (capsMatch) {
                            ...
                            sel.removeAllRanges(); sel.addRange(nr);
                            return; // Skip further shortcuts for this word
                        }

                        // Robust suffix-based matching for shortcuts
                        // Fix: Explicitly type entries to ensure it is correctly recognized as iterable
                        const matchedShortcut = Array.from(shortcutsMap as Iterable<[string, string]>)
                            .sort((a, b) => b[0].length - a[0].length)
                            .find(([key]) => {
                                if (!textBeforeCursor.endsWith(key)) return false;
                                
                                // Word key check: punctuation or spaces must precede letters/numbers keys
                                const isWordKey = /^[a-zA-Z0-9']+$/.test(key);
                                if (isWordKey) {
                                    const charBefore = textBeforeCursor.length > key.length 
                                        ? textBeforeCursor[textBeforeCursor.length - key.length - 1] 
                                        : ' ';
                                    return /[ \s(\[{“‘]/.test(charBefore);
                                }
                                return true; // Complex punctuation strings match instantly
                            });

                        if (matchedShortcut) {
                            const [key, value] = matchedShortcut;
                            const startIndex = offset - 1 - key.length;
                            node.textContent = textContent.substring(0, startIndex) + value + textContent.substring(offset - 1);
                            const newCaretPos = startIndex + value.length + 1;
                            const nr = document.createRange(); nr.setStart(node, newCaretPos); nr.collapse(true);
                            sel.removeAllRanges(); sel.addRange(nr);
                        }
                    }
    ...
    useEffect(() => {
        const handleGlobalKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === 'PageDown') {
                ...
                const totalSpreads = layout.stride > 0 ? Math.ceil((editorContainerRef.current?.scrollWidth || 0) / layout.stride) : 0;
                
                if (currentSpread >= totalSpreads - layout.columns && chapters.length > 0) {
                    const currentIndex = chapters.findIndex(c => c.id === activeChapterId);
                    if (currentIndex < chapters.length - 1) {
                        onActiveChapterIdChange(chapters[currentIndex + 1].id);
                    }
                } else {
                ...
                if (currentSpread <= 0 && chapters.length > 0) {
                    const currentIndex = chapters.findIndex(c => c.id === activeChapterId);
                    if (currentIndex > 0) {
                        onActiveChapterIdChange(chapters[currentIndex - 1].id);
                        setTimeout(() => {
                            if (editorContainerRef.current) {
                                editorContainerRef.current.scrollTo({ left: editorContainerRef.current.scrollWidth, behavior: 'instant' });
                            }
                        }, 50);
                    }
                } else {
    ...
    const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
        ...
        if (e.key === 'Home') {
            ...
            if (firstChapter && activeChapterId !== firstChapter.id) {
                onActiveChapterIdChange(firstChapter.id);
                // The layout effect will handle scrolling to 0
            } else if (editorContainerRef.current) {
                editorContainerRef.current.scrollTo({ left: 0, behavior: 'smooth' });
            }
            if (editorRef.current) {
                const range = document.createRange();
                const sel = window.getSelection();
                if (sel) {
                    try {
                        ...
                        sel.addRange(range);
                        editorRef.current.focus();
                    } catch (err) { console.warn("Home key selection failed", err); }
                }
            }
            return;
        }
        if (e.key === 'End') {
            ...
            if (lastChapter && activeChapterId !== lastChapter.id) {
                onActiveChapterIdChange(lastChapter.id);
                // Need to scroll to the end after chapter load
                setTimeout(() => {
                    if (editorContainerRef.current) {
                        editorContainerRef.current.scrollTo({ left: editorContainerRef.current.scrollWidth, behavior: 'smooth' });
                    }
                }, 100);
            } else if (editorContainerRef.current) {
                editorContainerRef.current.scrollTo({ left: editorContainerRef.current.scrollWidth, behavior: 'smooth' });
    ...
    const handleTTSPlay = useCallback(async () => {
        ...
        if (ttsStatus === 'paused' && ttsAudioElementRef.current) {
            try {
                await ttsAudioElementRef.current.play();
                setTtsStatus('playing'); return;
            } catch (e) { console.warn("Resumed play failed", e); }
        }
        setTtsStatus('loading'); const selection = window.getSelection(); let textToRead = '';
        if (selection && selection.toString().trim().length > 0 && editorRef.current?.contains(selection.anchorNode)) {
            ...
            const div = document.createElement('div'); div.innerHTML = activeChapter.content; textToRead = div.innerText;
        }
        ...
        try {
            ...
            if (ttsAudioElementRef.current) {
                ttsAudioElementRef.current.pause(); ttsAudioElementRef.current.src = ""; ttsAudioElementRef.current = null;
            }
            const bytes = decode(base64Audio); const audioUrl = createWavUrl(bytes, 24000); const audio = new Audio(audioUrl);
            audio.playbackRate = settings.ttsSpeed || 1.0; audio.volume = settings.ttsVolume ?? 1.0; audio.preservesPitch = true;
            audio.onended = () => { setTtsStatus('idle'); URL.revokeObjectURL(audioUrl); };
            audio.onerror = () => { setTtsStatus('error'); URL.revokeObjectURL(audioUrl); };
            await audio.play(); ttsAudioElementRef.current = audio; setTtsStatus('playing');
        } catch (e) {
            console.error("TTS System Error:", e); setTtsStatus('error'); setTimeout(() => setTtsStatus('idle'), 3000);
        }
    }, [activeChapter.content, ttsStatus, settings]);

    const handleTTSPause = useCallback(() => {
        if (ttsAudioElementRef.current) { ttsAudioElementRef.current.pause(); setTtsStatus('paused'); }
    }, []);
    ...
    useLayoutEffect(() => {
        const isChapterChange = prevChapterId.current !== activeChapter.id;
        if (editorRef.current && activeChapter.content && editorRef.current.innerHTML !== activeChapter.content && !isLocalUpdate.current) {
            editorRef.current.innerHTML = activeChapter.content;
        }

        if (isChapterChange) {
            prevChapterId.current = activeChapter.id;
            if (editorContainerRef.current) {
    ...
    useEffect(() => {
        setIsToolbarAnimating(true); const timer = setTimeout(() => setIsToolbarAnimating(false), 300);
        return () => clearTimeout(timer);
    }, [isFocusMode]);
    ...
    return (
        <div className="flex h-full overflow-hidden antialiased" style={{ backgroundColor: "transparent" }}>
            ...
            `}</style>
            {notification && (
                <div className="fixed top-6 left-1/2 transform -translate-x-1/2 z-[9999] px-6 py-2 rounded-full shadow-xl toast-enter flex items-center gap-2 backdrop-blur-md border border-white/20" style={{ backgroundColor: settings.accentColor || settings.successColor, color: 'var(--app-text)' }}><span className="font-bold text-sm">{notification}</span></div>
            )}
                ...
                <div className="flex-grow relative min-h-0 overflow-hidden">
                        ...
                        <div className="absolute left-0 top-0 w-full z-30 pointer-events-none" style={{ paddingLeft: `${layout.sideMargin}px`, paddingRight: `${layout.sideMargin}px`, paddingTop: '1.25rem' }}>
                            <div className="flex items-center gap-2 pointer-events-auto border-b border-dashed pb-0.5 transition-colors focus-within:border-opacity-100" style={{ color: settings.textColor, borderColor: `${settings.textColor}40`, width: layout.columns === 2 ? `${layout.colWidth}px` : '100%' }}>
                                ...
                                <input
                                    ...
                                    onChange={(e) => handleChapterDetailsChange(activeChapter.id, { title: e.target.value })}
                                    className="bg-transparent border-none outline-none text-lg font-bold tracking-tight w-full p-0"
                    ...
                    {(ttsStatus === 'playing' || ttsStatus === 'paused') && activeModal !== 'readAloud' && (
                        <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-50 rounded-full shadow-lg flex items-center px-3 py-2 gap-3 border backdrop-blur-md transition-all duration-300 ease-in-out" style={{ backgroundColor: `${settings.toolbarBg}E6`, borderColor: settings.toolbarInputBorderColor, color: settings.toolbarText }}>
                            ...
                            <button onClick={handleTTSStop} className="p-2 rounded-full hover:bg-black/10 transition-colors text-app-danger" title="Stop"><StopIcon className="h-6 w-6"/></button>
                        </div>
                    )}
