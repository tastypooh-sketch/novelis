...
interface ChapterTileProps {
    ...
    isReorderingPending: boolean;
}

const ChapterTile: React.FC<ChapterTileProps> = React.memo(({
    chapter, allCharacters, snippets, settings, isExpanded, isSelected, onSelect, onToggleExpand, onUpdate, onDeleteRequest, draggableProps, isDragging, tileBackgroundStyle, scrollContainerRef, zoomLevel, isAnyExpanded, expandedChapterId, isReorderingPending
}) => {
    ...
    const handleTitleUpdate = () => {
        ...
        if (isReorderingPending && chapter.title === 'Chapter' && trimmedTitle !== 'Chapter') {
            dialog.alert("Confirm updated chapter arrangements first", "Arrangement Pending");
            setLocalTitle(chapter.title);
            return;
        }
    ...
    const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (isReorderingPending) {
            dialog.alert("image edits cannot be saved until pending arrangement changes are confirmed.", "Arrangement Pending");
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {chapter.previousDetails ? (
                                ...
                                <div className="flex items-center gap-2 relative">
                                    <button
                                        onClick={(e) => { e.stopPropagation(); setShowUpdateConfirm(!showUpdateConfirm); }}
                                        className="px-4 py-2 rounded-lg text-sm font-bold flex items-center transition-colors shadow-lg active:scale-95"
                                        style={{ backgroundColor: actionButtonBg, color: actionButtonText }}
                                    >
                                        <BrushIcon className="h-4 w-4 mr-2" /> Update from Manuscript
                                    </button>
                                    <button
                                        onClick={(e) => { e.stopPropagation(); handleSendBriefToManuscript(); }}
                                        className="px-4 py-2 rounded-lg text-sm font-bold flex items-center transition-colors shadow-lg active:scale-95 relative"
                                        style={{ backgroundColor: actionButtonBg, color: actionButtonText }}
                                    ...
                                    >
                                        ...
                                        {showToast && (
                                            <div className="absolute -top-12 left-1/2 -translate-x-1/2 px-3 py-1.5 rounded-lg bg-green-600 text-white text-[10px] font-bold shadow-xl animate-in fade-in slide-in-from-bottom-2 duration-300 whitespace-nowrap z-[100] border border-white/20">
                                                Brief sent to Manuscript!
                                            </div>
                                        )}
                                    </button>
                                    {showUpdateConfirm && (
                                        <div className="absolute bottom-full left-0 mb-2 w-72 p-4 rounded-xl shadow-2xl text-xs z-50 border border-white/10 animate-in fade-in zoom-in-95 duration-200" style={{backgroundColor: settings.dropdownBg, color: settings.toolbarText}}>
                                            <div className="flex justify-between items-start mb-2">
                                                <h4 className="font-bold uppercase tracking-wider text-[10px] opacity-60">Confirm Intelligence Update</h4>
                                                <button onClick={() => setShowUpdateConfirm(false)} className="opacity-40 hover:opacity-100">✕</button>
                                            </div>
                                            ...
                                            <div className="mt-4 flex gap-2">
                                                <button 
                                                    onClick={() => {
                                                        handleUpdateFromManuscript();
                                                        setShowUpdateConfirm(false);
                                                    }}
                                                    className="flex-grow py-2 rounded-lg font-bold transition-all active:scale-95 shadow-md"
                                                    style={{ backgroundColor: settings.accentColor, color: getContrastColor(settings.accentColor) }}
                                                >
                                                    Confirm Update
                                                </button>
                                                <button 
                                                    ...
                                                    className="px-4 py-2 rounded-lg font-bold opacity-60 hover:opacity-100 transition-all border"
                                                    style={{ backgroundColor: settings.toolbarButtonBg, color: settings.toolbarText, borderColor: settings.toolbarInputBorderColor }}
                                                >
                                                    Cancel
                                                </button>
                                            </div>
    ...
    return (
        <div
            ...
            onDrop={handleCharacterDrop}
            {...draggableProps}
            className={`relative aspect-[3/4] flex flex-col rounded-lg shadow-lg transition-all duration-300 border-4 overflow-hidden ${isDragging || (isAnyExpanded && !isExpanded && expandedChapterId !== chapter.id) ? 'opacity-40 grayscale-[0.5] scale-[0.98]' : 'opacity-100 scale-100'}`}
            style={{
                borderColor: isSelected ? settings.accentColor : (chapter.accentStyle === 'outline' ? accentColor : 'transparent'),
                color: tileTextColor,
                ...backgroundStyle,
        ...
        try {
            ...
            const images = await Promise.all(chaptersWithPhotos.map(ch => {
                return new Promise<{ img: HTMLImageElement; chapter: IChapter }>((resolve, reject) => {
                    const img = new Image();
                    img.crossOrigin = "anonymous";
                    img.onload = () => resolve({ img, chapter: ch });
                    img.onerror = reject;
                    img.src = ch.photo!;
                });
            }));
            ...
            dialog.error("Failed to generate storyboard collage. Please ensure all images are accessible.", "Export Failed");
        } finally {
            setIsSyncing(false);
        }
    };
                                    ...
                                    >
                                        {acts[actNum].map(ch => (
                                            <ChapterTile 
                                                ...
                                                isAnyExpanded={!!expandedChapterId}
                                                expandedChapterId={expandedChapterId}
                                                isReorderingPending={isDirty}
