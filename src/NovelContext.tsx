...
export type Action =
  ...
  | { type: 'UPDATE_CONCEPT'; payload: string }
  ...
  | { type: 'UPDATE_REWRITE_STATE', payload: Partial<RewriteState> }
  | { type: 'TRACK_SPELLING_ERROR', payload: string }
  | { type: 'ADD_TO_AUTOCORRECT', payload: { word: string; correction: string } }
...
const novelReducer = (state: INovelState, action: Action): INovelState => {
  return produce(state, draft => {
    switch (action.type) {
      ...
      case 'UPDATE_CONCEPT': {
        draft.highLevelConcept = action.payload;
        break;
      }
      case 'UPDATE_SUMMARY': {
        draft.highLevelSummary = action.payload;
        break;
      }
      ...
      case 'UPDATE_WHAT_IF_STATE':
        ...
        break;
      case 'UPDATE_REWRITE_STATE':
        Object.assign(draft.rewriteState, action.payload);
        break;
      case 'TRACK_SPELLING_ERROR':
        draft.sessionSpellingErrors[action.payload] = (draft.sessionSpellingErrors[action.payload] || 0) + 1;
        break;
      case 'ADD_TO_AUTOCORRECT':
        draft.autoCorrectDictionary[action.payload.word] = action.payload.correction;
        break;
      case 'CLEAR_SESSION_ERRORS':
        draft.sessionSpellingErrors = {};
        break;
