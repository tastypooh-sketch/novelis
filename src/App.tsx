...
import { RewriteModal } from './components/common/RewriteModal';
import { AutoCorrectReviewModal } from './components/common/AutoCorrectReviewModal';
...
const App: React.FC = () => {
    ...
    const [spellingSuggestions, setSpellingSuggestions] = useState<{ word: string; correction: string }[]>([]);
    ...
    useEffect(() => {
        if (chapters.length > 0 && (isFirstLoad.current || !chapters.find(c => c.id === activeChapterId))) {
    ...
    const analyzeSpellingErrors = useCallback(async () => {
        if (!chapters.length || !settings.geminiApiKey) return [];
        
        // Take a sample of the most recent chapter text
        const activeChapter = chapters.find(c => c.id === activeChapterId) || chapters[chapters.length - 1];
        const textToAnalyze = activeChapter.content.replace(/<[^>]*>/g, ' ').substring(0, 5000);

        try {
            const prompt = `You are a spelling and grammar expert. Analyze the following novel passage for RECURRING spelling errors or obvious typos that the author might want to add to an auto-correct dictionary.

Focus only on clear mistakes, not stylistic choices or unique character names (unless they look like common typos).
...
Return your response as a JSON array of objects, where each object has "word" (the typo) and "correction" (the fixed version). Only include high-confidence corrections. If no clear errors are found, return an empty array [].`;
            ...
            console.error("Spelling analysis failed", e);
            return [];
        }
    }, [chapters, activeChapterId, settings.geminiApiKey]);
    ...
    const handleSaveAndExit = async () => {
        const suggestions = await analyzeSpellingErrors();
        if (suggestions && suggestions.length > 0) {
            setSpellingSuggestions(suggestions);
            setIsReviewModalOpen(true);
            // We wait for the user to finish review or close it before actually exiting
            return;
        }
    ...
    useEffect(() => {
        // Ensure activeChapterId is valid if chapters change, default to last chapter if invalid or first load
        if (chapters.length > 0 && !chapters.find(c => c.id === activeChapterId)) {
            setActiveChapterId(chapters[chapters.length - 1].id);
        }
    }, [chapters, activeChapterId]);
                ...
                <div className={`relative z-10 h-full w-full flex flex-col ${isFullscreen ? '' : 'pt-8'}`}> {/* Conditional Padding Top */}
                    ...
                    {whatIfState.isOpen && (
                        <WhatIfModal
                            ...
                            onRegenerate={() => onGenerateWhatIf(whatIfState.originalText || '', whatIfState.context || '')}
                        />
                    )}
                    {rewriteState.isOpen && (
                        <RewriteModal
                            state={rewriteState}
                            settings={settings}
                            onClose={() => dispatch({ type: 'UPDATE_REWRITE_STATE', payload: { isOpen: false } })}
                            onRegenerate={() => onRewriteParagraphs(rewriteState.originalText || '', rewriteState.context || '')}
                        />
                    )}
                    {isExitModalOpen && (
                        <ExitModal 
                    ...
                    {isReviewModalOpen && (
                        <AutoCorrectReviewModal
                            settings={settings}
                            suggestions={spellingSuggestions}
                            onAccept={(word, correction) => {
                                dispatch({ type: 'ADD_TO_AUTOCORRECT', payload: { word, correction } });
                            }}
                            onIgnore={() => {}}
                            onClose={async () => {
                                setIsReviewModalOpen(false);
                                // After review, proceed with save and exit
                                const saved = await handleSaveToFolder();
                                if (saved) handleExitWithoutSaving();
                            }}
                        />
                    )}
