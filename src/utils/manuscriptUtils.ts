...
const processNodeForRtf = (node: Node): string => {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        switch (tagName) {
            ...
            case 'p': case 'div': return `${childrenRtf}\\par `;
            case 'br': return '\\line ';
...
export const generateRtfForChapters = (chapters: IChapter[]): string => {
    const header = `{\\rtf1\\ansi\\ansicpg1252\\deff0\\nouicompat\\deflang1033`;
    const fontTbl = `{\\fonttbl{\\f0\\fnil\\fcharset0 Times New Roman;}}`;
    // Standard manuscript spacing: \sl480\slmult1 is double spacing, \sl360\slmult1 is 1.5 spacing
    // \sa120 adds 6 points of space after each paragraph
    const styles = `\\viewkind4\\uc1\\pard\\sl360\\slmult1\\sa120\\f0\\fs24`;
